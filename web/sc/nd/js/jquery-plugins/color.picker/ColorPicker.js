...
/* Handle keystrokes. */
	...
	showFor: function(control) {
	    ...
	    inst._colorPickerDiv.css('position', /*($.blockUI ? 'static' : (isFixed ? 'fixed' : 'absolute'))*/'absolute'   ).css('left',  ($.colorPicker._pos[0]-300) + 'px').css('top', $.colorPicker._pos[1]+1 + 'px');
	...
	hideColorPicker: function(s) {
		...
		if (this._colorpickerShowing)
		{
			...
			{
				$.unblockUI();
				$(/*'body'*/$('#colabopad-frame')).append(this._colorPickerDiv);
			}
...
$(document).ready(function() {
    ...
    $($('#colabopad-frame')).append($.colorPicker._colorPickerDiv).mousedown($.colorPicker._checkExternalClick);
});
