(function($) {
    var imageUrl='images/misc/color.png';
    $.iColorShow=function (elm,id2){
        //var eICP=$("#"+id2).position();
        $("#iColorPicker").css({/*'top':eICP.top+($(elm).outerHeight())+15+"px",'left':(eICP.left)+15+"px",*/'position':'absolute'}).fadeIn("fast");
              
        
        $("#iColorPickerBg").css({'position':'absolute','top':0,'left':0,'width':'100%','height':'100%'}).fadeIn("fast");
        var def=$(elm).val();
            ...
            for(j=0;j<tblChilds.length;j++){
                ...
                for(k=0;k<tblCells.length;k++){
                    $(tblChilds[j].childNodes[k]).unbind().mouseover(function(a){
                        ...
                        $('#colorPreview span').text(aaa)}).click(function(){
                            var aaa="#"+$(this).attr('hx');
                            $(elm).val(aaa).css("background",aaa);
                            $(elm).change();
                            $("#iColorPickerBg").hide();
    ...
    $.fn.makeColorPicker = function(){
         this.each(function (i) {
              var _this = this;
              try{
                   $('#colorPreview').css({'height':'50px'});
                   var uuid = new Date().getTime();
                   $(this).after('<input type="image" id="icp_'+uuid+'" src="'+imageUrl+'" style="border:0;margin:0 0 0 3px" align="absmiddle" >').css("backgroundColor",$(this).val());

                   
                   $('#icp_'+uuid).click(function(evt){
                       
                       if(typeof this.positioned == "undefined"){
                           this.positioned = true;
                            $( "#iColorPicker" ).position({
                                of: this,
                                my: 'left top',
                                at: 'left bottom'/*,
                                offset: $( "#offset" ).val(),
